...
void CLUEAlgo::makeClusters(){
  ...
  // start clustering
//  std::cout << "Start prepareDataStructures" << endl;
//  auto start = std::chrono::high_resolution_clock::now();
  prepareDataStructures(allLayerTiles);
//  auto finish = std::chrono::high_resolution_clock::now();
//  std::chrono::duration<double> elapsed = finish - start;
//  std::cout << "Finish prepareDataStructures" << endl;

 // std::cout << "Start calculateLocalDensity" << endl;
 // start = std::chrono::high_resolution_clock::now();
  calculateLocalDensity(allLayerTiles);
 // finish = std::chrono::high_resolution_clock::now();
 // elapsed = finish - start;
//  std::cout << "--- calculateLocalDensity:     " << endl;

 // std::cout << "Start calculateDistanceToHigher" << endl;
//  start = std::chrono::high_resolution_clock::now();
  calculateDistanceToHigher(allLayerTiles);
//  finish = std::chrono::high_resolution_clock::now();
//  elapsed = finish - start;
//  std::cout << "--- calculateDistanceToHigher: " << endl;
...
void CLUEAlgo::findAndAssignClusters(){
  ...
  for(unsigned int i = 0; i < points_.n; i++) {
    ...
    if (isSeed) {
      ...
      nClusters++;
      cout<<"Point delta is: "<<points_.delta[i]<<" and rho is : "<<points_.rho[i]<<endl;
