#ifndef _SIPM_CALOCLUSTER_
#define _SIPM_CALOCLUSTER_
#include "TH1.h"
#include "TF1.h"
#include "TCanvas.h"
#include "CaloBar.h"
#include "CaloCrossBar.h"
#include "CaloCrossCluster.h"
#include <stdlib.h>

#include <algorithm>

using namespace std;
class SiPM_CaloCluster{

public: 
  SiPM_CaloCluster() {getEcalpar(); vector<CaloBar> getSiPMNeighbor(CaloBar& _ibar); };

  void Clear() { Bars.clear(); pos.SetXYZ(0.,0.,0.); phi=0; z=0; chi2=-99; alpha=-99; beta=-99;}
  vector<CaloBar> getBars() { return Bars; }

 void getEcalpar(){

    ifstream file("/afs/ihep.ac.cn/users/z/zhaoxiao/cms/CEPCSW/Digitisers/FanEcalDigi/script/Ecal_Rotated_parameter.txt");
    int i;
    string s;
    string name;
    double value;

    for(i=0;getline(file,s);++i){
        istringstream sin(s);
        sin>>name>>value;
        if(name == "Ecal_crystal_y_width"){
           Ecal_crystal_y_width = value;
        }
        if(name == "Ecal_crystal_rotate_angle"){
           Ecal_alpha = value;
        }
        if(name == "Ecal_crystal_envelope_length"){
           Ecal_crystal_envelope_length = value;
        }
        if(name == "Ecal_barrel_outer_radius_redef"){
           Ecal_rmax = value;
        }
        if(name == "Ecal_barrel_inner_radius"){
           Ecal_rmin = value;
        }
        if(name == "Ecal_barrel_thickness"){
           Ecal_barrel_thickness = value;
        }
        if(name == "Ecal_barrel_outer_radius"){
           Ecal_barrel_outer_radius = value;
        }
        if(name == "Ecal_barrel_half_length"){
           Ecal_barrel_half_length = value;
        }
        if(name == "nphi"){
           Ecal_nphi = int(value);
        }
        if(name == "numberZ"){
           Ecal_nz = value;
        }
        if(name == "zhalf"){
           Ecal_zhalf = value;
        }
      }
   }
  
  void SiPM_FitProfile(){
  //  TCanvas *c1 = new TCanvas();
    double initPhi = getSiPMPhiStart();  
    double X0 = 11.2;   //Radiation length, unit: mm
    TH1D *h_hitz = new TH1D("h_hitz", "h_hitz", 30, 0, 30);
    for(int i=0; i<Bars.size(); i++){
//      double relR = (Bars[i].getCrystal()*180./571 - initPhi)*1900*3.1415/(180*X0);
  	double relR = (Bars[i].getCrystal()*ang_pi/int(Ecal_nphi/2) - initPhi)*Ecal_rmin*pi/(ang_pi*X0);
        h_hitz->Fill(relR, Bars[i].getSiPMEnergy());
    }    
    h_hitz->Draw();

    TF1 *func = new TF1("fc1", "[2]*[1]*pow([1]*x, ([0]-1))*exp(-[1]*x)/TMath::Gamma([0])", 0, 30);
    func->SetParName(0, "alpha");
    func->SetParName(1, "beta");
    func->SetParName(2, "E0");
    func->SetParameter(0, 4.8);
    func->SetParameter(1, 0.5);
    func->SetParameter(2, 10);
    h_hitz->Fit("fc1","Q","",0,30);
  //  c1->SaveAs("/afs/ihep.ac.cn/users/z/zhaoxiao/cms/CEPCSW/Detector/DetCRD/scripts/data/h_hitz.png");

    chi2 = func->GetChisquare()/func->GetNDF();
    alpha = func->GetParameter(0);
    beta = func->GetParameter(1);

    delete h_hitz;
  //  delete c1;
  }
 
  double getSiPMPhiStart(){
    vector<int> phiID; phiID.clear();
    double totE = getSiPMEnergy();
    for(int i=0; i<Bars.size(); i++)
      if(Bars[i].getSiPMEnergy()>totE*0.01) phiID.push_back(Bars[i].getCrystal());
    sort(phiID.begin(), phiID.end());
    if(phiID.size()==0) return -99;
    return (double)phiID[0]*180./int(Ecal_nphi/2) ; //571.int(Ecal_nphi/2.)
  }

  double getSiPMPhiStartFit(){
    SiPM_FitProfile();
    double totE = getSiPMEnergy();
    double avePhi = 0;
    for(int i=0; i<Bars.size(); i++) avePhi += Bars[i].getSiPMEnergy()*Bars[i].getCrystal()*(180./int(Ecal_nphi/2))/totE;//571. 

    double X0 = 11.2;   //Radiation length, unit: mm
    double depth_Phi = (alpha/beta)*X0/Ecal_rmin * (ang_pi/pi);  // 
    return avePhi - depth_Phi;
  }

  double getSiPMZ(){
    double aveZ=0;
    double totE = getSiPMEnergy();
    for(int i=0; i<Bars.size(); i++) aveZ += Bars[i].getSiPMEnergy()*Bars[i].getPosition().z()/totE; 
    return aveZ; 
  }

  double getSiPMAvePhi(bool useEnergy = true){
    double avephi=0;
    double num = 0;
    double subTotE = getSubEnergy();
    for(int i=0; i<Bars.size(); i++) {
	if (Bars[i].getSiPMEnergy() > 0.05) {
	  //  subTotE += Bars[i].getSiPMEnergy();
	    if(useEnergy){
	    avephi += (Bars[i].getCrystal()*(180./int(Ecal_nphi/2))*Bars[i].getSiPMEnergy())/subTotE;
	    }else{
	     avephi += Bars[i].getCrystal()*(180./int(Ecal_nphi/2));
 	    }
	num+=1;
	}	
    }
    if(!useEnergy){avephi = avephi/num;}
    return avephi;
  }

  double getSiPMEnergy(){
    double totE = 0;
    for(int i=0; i<Bars.size(); i++){
     totE += Bars[i].getSiPMEnergy();
     }
    return totE; 
  }
  
  double getSubEnergy(){
    double subTotE = 0;
    for(int i=0; i<Bars.size(); i++) {
	if(Bars[i].getSiPMEnergy() > 0.05) subTotE += Bars[i].getSiPMEnergy();
    }
    return subTotE;
  }
//=================================21/21==================================
 
  vector<int> getSiPMClusModule(){
     vector<int> module_ID; module_ID.clear();
     for(int i=0; i<Bars.size(); i++){
	module_ID.push_back(Bars[i].getModule());
     }
     sort(module_ID.begin(),module_ID.end());
     module_ID.erase(unique(module_ID.begin(),module_ID.end()),module_ID.end());
     return module_ID;
  }
 
 vector<CaloCrossBar> getSiPMCrossPoint( CaloBar& _ibar ){
    dd4hep::Position c_pos;
    CaloCrossBar c_bar;
    vector<CaloCrossBar> bars; bars.clear();
    CaloCrossCluster clus_bar; 
 
    double D = sqrt(pow(Ecal_rmax,2)-pow(Ecal_rmin*sin(Ecal_alpha*toRad),2))-Ecal_rmin*cos(Ecal_alpha*toRad);
    double phi = acos((pow(Ecal_rmax,2)+pow(Ecal_rmin,2)-pow(D,2))/(2*Ecal_rmin*Ecal_rmax))*toAng;//Rad zhongxinjiao
    double eta = ang_pi - Ecal_alpha;
    double O_phi,M_phi,delta_phi = 0;
    double bar_R,bar_phi,bar_z,bar_E = 0;
    double tot_BarE = 0;

    for(int i=0; i<Bars.size(); i++) {  
      if(Bars[i].getSiPMEnergy()>0.03 && _ibar.getSiPMEnergy()>0.03 && Bars[i].isNextModule(_ibar)){
     //   cout<<"Bsr SiPM energy: "<<Bars[i].getSiPMEnergy()<<endl;
        O_phi = _ibar.getCrystal()*(180./int(Ecal_nphi/2));
        M_phi = Bars[i].getCrystal()*(180./int(Ecal_nphi/2));
        delta_phi = abs(O_phi-M_phi);
     //   cout<<"delta_phi = "<<delta_phi<<" ; delta_module = "<<_ibar.getModule()-Bars[i].getModule()<<endl;
        if(delta_phi==0){
           bar_R = Ecal_rmin;
           bar_phi = O_phi;
           bar_z = (_ibar.getPosition().z()+Bars[i].getPosition().z())/2.;
	   bar_E = _ibar.getSiPMEnergy()+Bars[i].getSiPMEnergy();
           c_pos.SetXYZ(bar_R,bar_phi,bar_z);
           c_bar.setPosition(c_pos);
           c_bar.setEnergy(bar_E);
	   bars.push_back(c_bar);
	   clus_bar.AddBar(c_bar);
        }
        if(delta_phi>0 && delta_phi<=2*phi){
	 //  cout<<"c1:"<<_ibar.getCrystal()<<" m1: "<<_ibar.getModule()<<" c2:"<<Bars[i].getCrystal()<<" m2:"<<Bars[i].getModule()<<endl;
         //  cout<<"delta_phi = "<<delta_phi<<" ; delta_module = "<<_ibar.getModule()-Bars[i].getModule()<<endl;           
           bar_R = Ecal_rmin*sin(eta*toRad)/sin((ang_pi-eta-delta_phi/2.)*toRad);
           if(O_phi<M_phi){
              bar_phi = O_phi+(delta_phi/2.);
           }else{
 	      bar_phi = M_phi+(delta_phi/2.);
           }
           
           bar_z = (_ibar.getPosition().z()+Bars[i].getPosition().z())/2.;
	   bar_E = _ibar.getSiPMEnergy()+Bars[i].getSiPMEnergy();
           c_pos.SetXYZ(bar_R,bar_phi,bar_z);
	   c_bar.setPosition(c_pos);
           c_bar.setEnergy(bar_E);
     //      cout<<"bar_R: "<<c_bar.getPositionR()<<" bar_phi: "<<bar_phi<<" bar_z: "<<bar_z<<" bar_E: "<<bar_E<<endl;   
           bars.push_back(c_bar);
	   clus_bar.AddBar(c_bar);
        }
	tot_BarE+=Bars[i].getSiPMEnergy();
     }
   }
   
   if(bars.size()>0){
     tot_BarE+= _ibar.getSiPMEnergy();
//     cout<<" tot_BarE: "<<tot_BarE<<endl;
     for(int k=0;k<bars.size();k++){
        double fac = tot_BarE/clus_bar.getEnergy();
	bars[k].setEnergy(bars[k].getEnergy()*fac);
     }
   }

     return bars;
 }
 

  double getSiPMFitAlpha() {return alpha;}
  double getSiPMFitBeta() {return beta;}
  double getSiPMFitChi2() {return chi2;}

  bool inSiPMCluster( CaloBar& _ibar ){
    for(int i=0; i<Bars.size(); i++) 
      if(_ibar==Bars[i]) return true; 
    return false; 
  }

  bool isSiPMNeighbor( CaloBar& _ibar ){
    if(inSiPMCluster(_ibar)) return false; 

    for(int i=0; i<Bars.size(); i++)
      if( Bars[i].isNeighbor(_ibar) ) return true; 
    
    return false; 
  }

  void AddSiPMBar( CaloBar& _ibar) { Bars.push_back(_ibar); }

  vector<CaloBar> getSiPMNeighbor(CaloBar& _ibar){
	n_Bars.clear();
    for(int i=0; i<Bars.size(); i++){
      if(Bars[i].isNeighbor(_ibar)){
	n_Bars.push_back(Bars[i]);
      }
    }
    return n_Bars;
  }
private: 
  vector<CaloBar> Bars;
  vector<CaloBar> n_Bars;
  TVector3 pos;
  dd4hep::Position c_pos;
  double phi;
  double z; 
  int id;
  double Ecal_crystal_y_width;
  double Ecal_barrel_inner_radius;//mm from ~/cms/CEPCSW/Detector/DetCRD/compact/Standalone/Dimensions_v01_01.xml
  double Ecal_barrel_outer_radius;
  double Ecal_barrel_thickness;
  double Ecal_barrel_half_length;
  double Ecal_crystal_envelope_length;
  double Ecal_zhalf;//Ecal_barrel_half_length_correct from ~/cms/CEPCSW/Detector/DetCRD/compact/CRD_common_v01/Ecal_Rotated_Crystal_v01_01.xml
  double Ecal_alpha;//Ecal_crystal_rotate_angle
  int Ecal_nphi;
  double Ecal_rmin;//Ecal_barrel_inner_radius
  double Ecal_rmax;//Ecal_barrel_outer_radius_redef
  int Ecal_nz;//numberZ

  
  double chi2;
  double alpha;
  double beta;

  const double pi = 3.1415926;
  double ang_pi = 180.;
  double toAng = ang_pi/pi;
  double toRad = pi/ang_pi;
  double nlayer = 10;
};

#endif
